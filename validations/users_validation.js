   ...
   try {
     ...
     //firstname validation
       ...
       if (typeof firstName !== "string")
         next(new AppError("firstname must be string", 400));
       if (firstName?.trim().length < 3 || firstName.length > 30)
         next(new AppError("firstname length must be betweeb 3 and 30", 400));
     ...
     //username validation
       ...
       if (duplicateUser)
         next(new AppError("this username already exist", 409));
     
     //gender validation
       const validGenderInput = ["male", "female", "not_set"];
       if (!validGenderInput.includes(gender))
         next(new AppError("non valid gender input", 400));
     
     //role validation
       const roleValidateInput = ["admin", "blogger"];
       if (!roleValidateInput.includes(role))
         next(new AppError("non valid role input", 400));
     next();
    
   } catch (error) {
      console.log(error);
      next(new AppError("internal error(validation catch error)",500))
   }
}
